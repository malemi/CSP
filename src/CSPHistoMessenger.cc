...
CSPHistoMessenger::CSPHistoMessenger(CSPHistoManager* manager)
:histoManager (manager)
{
  histoDir = new G4UIdirectory("/csp/histo/");
  histoDir->SetGuidance("histograms control");

  factoryCmd = new G4UIcmdWithAString("/csp/histo/setFileName",this);
  factoryCmd->SetGuidance("set name for the histograms file");

  typeCmd = new G4UIcmdWithAString("/csp/histo/setFileType",this);
  typeCmd->SetGuidance("set histograms file type: hbook, root, XML");
  typeCmd->SetCandidates("hbook root XML");

  optionCmd = new G4UIcmdWithAString("/csp/histo/setFileOption",this);
  optionCmd->SetGuidance("set option for the histograms file");
  
  histoCmd = new G4UIcommand("/csp/histo/setHisto",this);
...
void CSPHistoMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
